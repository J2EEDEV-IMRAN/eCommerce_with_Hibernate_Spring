...
public class UserJpaDao extends GenericJpaDao<UserEntity, Long> implements UserDao {
	...
	public void setUserActive(UserEntity user)
	{
		String uname=user.getUserName();
	   Query query=getEntityManager().createQuery("update "+getPersistentClass().getSimpleName()
	     +" u set u.userStatus=:userStatus where u.userName=:uname").setParameter("userStatus",UserStatus.ACTIVE).setParameter("uname", uname);
	    query.executeUpdate();
